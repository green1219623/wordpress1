...
function my_script_init()
{
    ...
    wp_enqueue_style('my', get_template_directory_uri() . '/css/reset.css', array(), '1.0.0', 'all');
...
function my_the_post_tags($id = 0)
{
    ...
    if ($tags) {
        foreach ($tags as $tag) {
            echo '<div class="single-tag-btn"><a href="' . esc_url(get_tag_link($tag->term_id)) . '">' . esc_html($tag->name) . '</a></div><!-- single-tag-btn -->';
        }
    }
}
...
function my_shortcode() {
    return '<div class="single-btn-wrap"><a class="single-btn" href="https://dev/contact/">お問い合わせはこちら</a></div><!-- /entry-btn -->';
    }
